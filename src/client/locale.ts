...
export const ZH: StudyDict = {
  ...
  // issue #8: artifact-card lifecycle
  'artifact.guess.revealed': '已揭晓——结论见导师的下一条回复',
  'artifact.fold': '收起卡片（笔记区仍保留）',
  'artifact.unfold': '展开卡片',
  'artifact.kind.compare_table': '对照表',
  'artifact.kind.code_walkthrough': '代码走查',
  'artifact.kind.diagram': '示意图',
  ...
  'boundary.done': '本课完成，皇冠到手！',
  'boundary.course.done': '整门课程完成！',
  'boundary.next.sub': '下一课是《{title}》——在课程栏点它进入。',
  'boundary.thread.sub': '下一课是《{title}》——点开始，就在这条线上接着讲。',
  'boundary.thread.point': '开始下一课',
  'boundary.thread.send': '这一课完成了，我们开始下一课《{title}》吧。',
  'boundary.last.sub': '全部课时都已掌握，复习会按计划回来。',
  'boundary.point': '在课程栏指出下一课',
  'boundary.dismiss': '收起',
  ...
  'import.success': '导入成功',
  // D8: the shared CodeBlock header + the render-failure boundaries
  'chat.copy': '复制',
  'chat.copied': '已复制',
  ...
  'threads.open': '{n} 条对话',
  'threads.now': '当前',
  'threads.new': '新建对话（下一条消息开新线）',
  'threads.actions': '线程操作',
  'rail.export.pack': '导出课程包（可再导入/分享的 Markdown）',
  'rail.export.done': '课程包已下载：{file}',
  'threads.rename': '重命名',
  'threads.archive': '归档',
  'threads.delete': '删除',
  'threads.delete.confirm': '删除「{name}」这条对话线？插件侧记录会移除；宿主会话列表中的原会话保留。',
  'review.banner': '正在复习：{lesson}',
  'review.threadTitle': '复习：{lesson}',
  'threads.label': '课时线程',
  'threads.scope': '跨课时连续对话',
  'threads.scope.on': '已开启',
  'threads.scope.off': '未开启',
  'threads.coverage': '覆盖',
  'ask.title': '导师想先确认一下',
  'ask.free': '在下方输入框直接回复即可',
  'ask.answered': '已回答',
  'meter.label': '上下文用量',
  ...
  'chat.think': '思考',
  'langview.original': '原文',
  'langview.translation': '译文',
  'msg.speak': '朗读',
  'note.pin': '置顶笔记',
  'note.unpin': '取消置顶',
  'note.locate': '回到原文',
  ...
  'settings.stats.streak': '连续 {days} 天 · 最长 {best} 天 · 冻结 {freeze}',
  'settings.stats.mastered': '已掌握 {count} 课 · 待复习 {due} 课',
  'settings.profile': '学习者画像',
  'settings.profile.hint': '只属于你的背景事实，用来调整怎么教——不进任何评分。AI 的建议也在这里处理，不打断对话。',
  'settings.profile.name': '称呼',
  'settings.profile.mbti': 'MBTI（快捷入口，自动展开为风格四维）',
  'settings.profile.interests': '兴趣点（顿号/逗号分隔，最多 8 个）',
  'settings.profile.interests.ph': '例如：天文、做菜、悬疑小说',
  'settings.profile.note': '想对导师说的话',
  'settings.profile.motive': '学习动机（只有你自己能设置）',
  'settings.profile.unset': '未设置',
  'settings.profile.save': '保存画像',
  'settings.profile.private': '画像只存本地 state.json，不会上传。',
  'settings.style.start': '讲解起点',
  'settings.style.analogy': '先具体实例建直觉',
  'settings.style.framework': '先框架后细节',
  'settings.style.interaction': '互动密度',
  'settings.style.dialogue': '边讲边问',
  'settings.style.lecture': '讲完一节再答疑',
  'settings.style.feedback': '反馈风格',
  'settings.style.direct': '直接纠错',
  'settings.style.encouraging': '先肯定再指正',
  'settings.style.pacing': '节奏',
  'settings.style.sequential': '按顺序推进',
  'settings.style.exploratory': '允许跳着学',
  'settings.suggest.head': 'AI 建议的',
  'settings.suggest.apply': '采纳',
  'settings.suggest.ignore': '忽略',
  'settings.about': '关于',
  ...
  'prompt.exam.bank': '请为「{section}」的章节测验出题:先用 study_lesson 读一遍本节课时,再按各课时的知识点用 study_exam_bank_apply 一次性提交整份题库(单选 5-15 题,kcTitle 必须用本节的概念标题,每题附一句话解析)。题库就绪后告诉我可以开考。',
...
export const EN: StudyDict = {
  ...
  // issue #8: artifact-card lifecycle
  'artifact.guess.revealed': 'Revealed — see the tutor reply below for the answer',
  'artifact.fold': 'Fold card (kept in the notes zone)',
  'artifact.unfold': 'Unfold card',
  'artifact.kind.compare_table': 'Compare table',
  'artifact.kind.code_walkthrough': 'Code walkthrough',
  'artifact.kind.diagram': 'Diagram',
  ...
  'boundary.done': 'Lesson complete — crown earned!',
  'boundary.course.done': 'Course complete!',
  'boundary.next.sub': 'Next up: {title} — click it in the course rail to continue.',
  'boundary.thread.sub': 'Next: {title} — continue right here in this thread.',
  'boundary.thread.point': 'Start the next lesson',
  'boundary.thread.send': "This lesson is wrapped up — let's start the next one, {title}.",
  'boundary.last.sub': 'Every lesson is mastered; reviews will come back on schedule.',
  'boundary.point': 'Point at the next lesson',
  'boundary.dismiss': 'Dismiss',
  ...
  'threads.scope': 'Continuous course thread',
  'threads.scope.on': 'on',
  'threads.scope.off': 'off',
  'threads.coverage': 'covers',
  'ask.title': 'The tutor wants a quick confirm',
  'ask.free': 'Answer in the composer below',
  'ask.answered': 'answered',
  'threads.open': '{n} threads',
  'threads.now': 'current',
  'threads.new': 'New thread (your next message starts it)',
  'threads.actions': 'Thread actions',
  'rail.export.pack': 'Export course pack (re-importable, shareable Markdown)',
  'rail.export.done': 'Course pack downloaded: {file}',
  'threads.rename': 'Rename',
  'threads.archive': 'Archive',
  'threads.delete': 'Delete',
  'threads.delete.confirm': 'Delete the thread "{name}"? The plugin-side record is removed; the original session stays in the host session list.',
  'review.banner': 'Reviewing: {lesson}',
  'review.threadTitle': 'Review: {lesson}',
  ...
  'attach.remove': 'Remove pending attachment',
  ...
  'chat.think': 'Think',
  'langview.original': 'Original',
  'langview.translation': 'Translation',
  'msg.speak': 'Speak',
  'note.pin': 'Pin note',
  'note.unpin': 'Unpin',
  ...
  'settings.stats.streak': 'Streak {days} days · best {best} · freezes {freeze}',
  'settings.stats.mastered': '{count} lessons mastered · {due} due',
  'settings.profile': 'Learner profile',
  'settings.profile.hint': 'Background facts only you can declare — they shape HOW you are taught, never any scoring. AI suggestions resolve here, never in the chat.',
  'settings.profile.name': 'Name',
  'settings.profile.mbti': 'MBTI (shortcut — expands into the style dims)',
  'settings.profile.interests': 'Interests (comma-separated, max 8)',
  'settings.profile.interests.ph': 'e.g. astronomy, cooking, mystery novels',
  'settings.profile.note': 'Note to the tutor',
  'settings.profile.motive': 'Learning motive (only you can set this)',
  'settings.profile.unset': 'Not set',
  'settings.profile.save': 'Save profile',
  'settings.profile.private': 'Stored in your local state.json only — never uploaded.',
  'settings.style.start': 'Explanation start',
  'settings.style.analogy': 'Concrete examples first',
  'settings.style.framework': 'Framework first',
  'settings.style.interaction': 'Interaction density',
  'settings.style.dialogue': 'Questions as we go',
  'settings.style.lecture': 'Lecture first, Q&A after',
  'settings.style.feedback': 'Feedback style',
  'settings.style.direct': 'Direct corrections',
  'settings.style.encouraging': 'Encouraging corrections',
  'settings.style.pacing': 'Pacing',
  'settings.style.sequential': 'Sequential',
  'settings.style.exploratory': 'Exploratory',
  'settings.suggest.head': 'AI suggested',
  'settings.suggest.apply': 'Accept',
  'settings.suggest.ignore': 'Ignore',
