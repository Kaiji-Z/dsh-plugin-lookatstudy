...
- **Releases run through GitHub Actions trusted publishing** (switched from manual `npm publish` after v0.9.0): verify green → bump version → commit + push → trigger `.github/workflows/publish.yml` (the Actions "Run workflow" button on main, or publish a GitHub Release). The job re-runs the full verify gate, then `npm publish --provenance` via OIDC — **no npm tokens stored anywhere**; the npm-side trusted-publisher binding (repo + `publish.yml` + environment `npm`) was configured once by the owner on npmjs.com. The `npm` GitHub environment may carry required reviewers — that approval IS the human gate the manual passkey flow used to provide. The local finish stays manual: `dsh plugin --profile web add dsh-plugin-lookatstudy@<ver>` (remove first to force a same-version switch).
