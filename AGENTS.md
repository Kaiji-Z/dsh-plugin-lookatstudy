...
- **Live testing** needs the harness checkout (`D:/Users/kaiji/vibecodingKJ/clones/deepseek-ai/deepseek-harness`, dsh-v0.1.3-alpha.1 since 2026-09-06; was dsh-v0.1.1-rc.2, and a sibling `../deepseek-harness` until 2026-09-01): the model key lives ONLY in its gitignored `.env` (referenced by name `Z_AI_API_KEY`, never committed anywhere; source it with `set -a; source <harness>/.env; set +a`), the web profile patch layer carries the GLM endpoint config, and the 3080 server is a background task. Since 0.1.3 the web server prints a per-boot `?token=` URL (auth expires after ~1 min per page load — grab the fresh token from the boot log for browser automation); plugin dashboard routes under `/lookatstudy/api/*` are NOT token-guarded by the host. Harness checkout jumps need `pnpm install` + `pnpm run build` before boot; the checkout's pinned tsdown does NOT expand brace globs in `entry` (the root config's `lib/types/{index,invariant,startup}.js` reads as one literal path — spell out the three entries). **Port 3080 is shared with stardeck smoke boots** (same web profile, `--patch cordis.smoke.yml`): the two projects' servers kill/block each other, and every "silent server death" in a session may be the co-tenant taking the port. IDENTIFY the listener by its command line (`Get-CimInstance Win32_Process`) before any `taskkill` — a bare netstat PID may be the OTHER project's live server (the stardeck smoke instance was once killed as a "zombie"). After killing, RE-CHECK netstat until :3080 shows no LISTENING before booting (boot-time EADDRINUSE means someone still holds it). For parallel testing, pass `--port 3081` instead of fighting over 3080.
