...
- `src/client/` — browser half: `views.tsx` (the tab + `transcriptRows`), `starter.tsx` (hero one-click), `settings.tsx` (settings.section page), `dock.tsx` (composer.dock status pill), `toolviews.tsx` (keyed tool.call.toolview cards), `locale.ts` (zh/en dictionaries + the module translator), `data.ts` (shared poll store), `styles.ts` (injected `--dsw-*` stylesheet), `icons.tsx` (ic_ds glyphs vendored byte-exact from ui-primitives + plugin-original learning-domain companions — lock/star/book/crown/bolt/power/pin/flame, NOT ic_ds). `src/commands.ts` — the host-side `/study` slash command (structural CommandInvocation; activation ordering identical to the dashboard's POST /api/active)
...
- The wide layout puts the tutor column left of center (fixed-preference rail `flex:0 1 260px`, blackboard takes ALL remaining width), so the floating composer must follow: StudyTab measures the tutor column's offset and puts `align-self:flex-start` + `--lks-composer-shift` on the host's `[data-composer-card]` anchor (the host's own stable composer hook). The follow class self-clears on narrow single-pane mode, hidden views, and unmount — the host's natural centering always returns. Lesson rows are real `<button>`s with `aria-disabled` (not the `disabled` attribute — Firefox drops title tooltips on truly disabled controls), and rail sections collapse off-frontier (`sectionDefaultOpen`, pure, tested).
