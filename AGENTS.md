...
- **Releases run through GitHub Actions trusted publishing** (switched from manual `npm publish` after v0.9.0): verify green → bump version → commit + push → **push a `v*` tag** (`git push origin v0.9.0`) — that tag push is the publish trigger; plain branch pushes never publish (the Actions "Run workflow" button works too). The job re-runs the full verify gate, then `npm publish --provenance` via OIDC — **no npm tokens stored anywhere**; the npm-side trusted-publisher binding (repo + `publish.yml`, environment field EMPTY — the workflow declares none) was configured once by the owner on npmjs.com. Pushing the tag IS the human gate. The local finish stays manual: `dsh plugin --profile web add dsh-plugin-lookatstudy@<ver>` (remove first to force a same-version switch).
