 ...
 *
 * Usage: node scripts/audit-ui.mjs <token> [--url http://127.0.0.1:3081]
 * (P16 will add --theme light|dark; today the panel is dark-on-light-host.)
 */
...
const audit = await page.evaluate(() => {
  const PANEL_SURFACES = new Set(['rgb(8, 9, 11)', 'rgb(12, 13, 15)', 'rgb(17, 17, 20)', 'rgb(26, 26, 29)', 'rgb(42, 43, 46)'])
  ...
  for (const el of root.querySelectorAll('*')) {
    ...
    const note = []
    if (bg !== null && bg.a > 0.85 && !PANEL_SURFACES.has(`rgb(${bg.r}, ${bg.g}, ${bg.b})`) && lum(bg.r, bg.g, bg.b) > 40 && !bgSaturatedLight) {
  ...
  return {
    theme: document.documentElement.getAttribute('data-theme') ?? '(no data-theme attr)',
...
process.exit(audit.suspectCount === 0 ? 0 : 1)
