...
// ——— B: the teach-tab language switcher ———
await page.locator('.lks14-viewtab').nth(0).dispatchEvent('click')
await page.waitForTimeout(1000)
const seg = await page.locator('[data-testid="langview"]').count()
probe('B: translated lesson shows the 原文/对照/译文 segmented control', seg === 1, `control=${seg}`)
let blockquotes = await page.locator('.lks14-prose blockquote').count()
...
probe('B: default view is 对照 (original + translation quotes interleaved)', blockquotes > 0 && hasZh > 0 && hasEn > 0, `quotes=${blockquotes} zh=${hasZh} en=${hasEn}`)

await page.locator('[data-testid="langview-original"]').dispatchEvent('click')
await page.waitForTimeout(600)
blockquotes = await page.locator('.lks14-prose blockquote').count()
hasZh = await page.locator('.lks14-prose', { hasText: '核心观点' }).count()
hasEn = await page.locator('.lks14-prose', { hasText: 'core idea' }).count()
probe('B: 原文 view drops the translation entirely', blockquotes === 0 && hasZh === 0 && hasEn > 0, `quotes=${blockquotes} zh=${hasZh} en=${hasEn}`)
...
hasZh = await page.locator('.lks14-prose', { hasText: '核心观点' }).count()
hasEn = await page.locator('.lks14-prose', { hasText: 'core idea' }).count()
probe('B: 译文 view shows the translation alone', blockquotes === 0 && hasZh > 0 && hasEn === 0, `quotes=${blockquotes} zh=${hasZh} en=${hasEn}`)
...
await page.locator('[data-testid="langview-bilingual"]').dispatchEvent('click')
