...
gate('bundle', () => {
  ...
  const required = [
    ...
    [host, 'bindLessonThread', 'thread-group binding (issue #11: many sessions per lesson, one active)'],
    [host, 'lesson-thread', 'the thread-management route (rename/archive/delete, issue #11 gear trio)'],
    [host, 'course-pack', 'course-pack export route (upstream exportPack alignment)'],
    [host, 'encWbi', 'bilibili wbi signing rides the CC subtitle path'],
    [host, 'study_import_url', 'the auto-routing URL import tool (github/arXiv/article/B站)'],
    [client, 'lks14-threadmenu-gear', 'the gear affordance on thread pills'],
    [client, 'thread-rename-input', 'the inline rename editor'],
    [client, 'course-export', 'the rail export-pack button'],
    [host, 'lessonThreads', 'thread groups ride the state feed and migration'],
    [client, 'threadAutoTitle', 'first-message thread auto-naming'],
    [client, 'lks14-threadmenu-plus', 'the plus-new thread row in the switcher'],
