...
- [x] D8 ErrorBoundary + 共享代码块 ✅ 2026-09-07：共享 CodeBlock 骑在 markdown 管线上（上游 v0.21 pre→CodeBlock 语义，HTML-管线版：围栏代码产出 lks-codeblock 卡=语言 chip（lang||'code' 大写 mono）+悬停复制钮头条 + 原 pre；chat/prose/notes 三区同源，locale 标签经管线 tr（host 侧 zh 默认/client 侧绑定 locale）；codeblock.ts 单一 document 级委托复制（Clipboard API、1500ms 已复制态、幂等 wire））。ErrorBoundary（上游 ErrorBoundary.tsx verbatim 语义：class + getDerivedStateFromError + componentDidCatch + retry；默认内联警示行（warning 图标+文案+下划线重试），ContentBoundary 富回退=警示+原文前 500 字预览+截断注+重试渲染）——包裹全部三个 innerHTML 危险区（chat 行、讲解 prose、笔记），超上游（上游只包了 notebook prose）。实测：markdown +1 测试（293 tests，verify PASS + 6 needles（含 host 侧管线），audit 0）+ 探针 probe-d8.mjs 4/4（种子笔记 ts 围栏→卡+chip+复制、复制标签 复制→已复制→复制 1500ms 往返、真实模型回复 python 围栏→chat 流同一卡、零 renderfail 区健康）。
