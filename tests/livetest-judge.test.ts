...
test('criteria: the committed judge-criteria.md parses into frozen C-ids', () => {
  ...
  assert.ok(cs.length >= 8, `expected at least 8 criteria, got ${cs.length}`)
  assert.deepEqual(cs.map(c => c.id), ['C1', 'C2', 'C3', 'C4', 'C5', 'C6', 'C7', 'C8'])
...
test('judgement: fenced, bare, and prose-embedded JSON all parse', () => {
  ...
  const payload = goodJudgement({ C1: 10, C2: 9, C3: 10, C4: 8, C5: 9, C6: 10, C7: 10, C8: 10 })
  for (const raw of [payload, '```json\n' + payload + '\n```', 'Here is my verdict.\n' + payload + '\nDone.']) {
    const j = parseJudgement(raw, criteria)
    assert.equal(j.criteria.length, 8)
  }
})
...
test('verdict: threshold rule over the good and the deficient transcript', () => {
  ...
  const pass = evaluateVerdict(parseJudgement(goodJudgement({ C1: 10, C2: 9, C3: 10, C4: 8, C5: 10, C6: 9, C7: 10, C8: 10 }), criteria))
  assert.equal(pass.pass, true)
  assert.equal(pass.minScore, 8)
  // The deficient fixture (filler concept, two answers, misgraded, inconsistent readout) fails.
  const fail = evaluateVerdict(parseJudgement(goodJudgement({ C1: 10, C2: 4, C3: 10, C4: 3, C5: 10, C6: 2, C7: 10, C8: 10 }), criteria))
  assert.equal(fail.pass, false)
  assert.equal(fail.minScore, 2)
  // Boundary: exactly the threshold passes; one below fails.
  assert.equal(evaluateVerdict(parseJudgement(goodJudgement({ C1: 8, C2: 8, C3: 8, C4: 8, C5: 8, C6: 8, C7: 10, C8: 10 }), criteria)).pass, true)
  assert.equal(evaluateVerdict(parseJudgement(goodJudgement({ C1: 8, C2: 7, C3: 8, C4: 8, C5: 8, C6: 8, C7: 10, C8: 10 }), criteria)).pass, false)
})
...
test('judgement: malformed model output is refused, not guessed at', () => {
  ...
  assert.throws(() => parseJudgement(goodJudgement({ C1: 10, C2: 9, C3: 10, C4: 8, C5: 9, C7: 10, C8: 10 }), criteria), /missing: C6/)
  assert.throws(() => parseJudgement(goodJudgement({ C1: 10, C2: 9, C3: 10, C4: 8, C5: 9, C9: 9, C7: 10, C8: 10 }), criteria), /unknown or missing/)
  assert.throws(() => parseJudgement(goodJudgement({ C1: 10, C2: 9, C3: 10, C4: 8, C5: 9, C6: '9' } as any), criteria), /integer 0-10/)
  assert.throws(() => parseJudgement(goodJudgement({ C1: 10, C2: 9, C3: 10, C4: 11, C5: 9, C6: 10, C7: 10, C8: 10 }), criteria), /integer 0-10/)
})
